...
#[cfg(test)]
mod security_tests {
    use super::{cfws, DEPTH_LIMIT};
    use std::vec;

    #[test]
    fn accepts_comments_below_the_depth_limit() {
        let nesting = usize::from(DEPTH_LIMIT - 1);
        let mut input = vec![b'('; nesting];
        input.extend(vec![b')'; nesting]);

        assert!(cfws(&input).is_some_and(|parsed| parsed.0.is_empty()));
    }

    #[test]
    fn rejects_comments_at_the_depth_limit() {
        let nesting = usize::from(DEPTH_LIMIT);
        let mut input = vec![b'('; nesting];
        input.extend(vec![b')'; nesting]);

        assert!(cfws(&input).is_none());
    }
}
